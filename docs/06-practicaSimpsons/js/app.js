...
function renderizar(lista) {
  ...
  lista.forEach(item => {
    ...
    edad.textContent = item.age ? `Edad: ${item.age}` : 'Edad desconocida';

    const fechaNacimiento = document.createElement('p');
    fechaNacimiento.textContent = item.birthdate ? `Fecha de nacimiento: ${item.birth_date}` : 'Fecha de nacimiento desconocida';
    
    const genero = document.createElement('p');
    genero.textContent = item.gender;
    
    const contenedorFrases = document.createElement('div');
    const frases= item.phrases;

    if (frases && frases.length > 0) {
        frases.forEach(fraseTexto => {
        const p = document.createElement('p');
        p.style.fontStyle = 'italic';
        p.textContent = `"${fraseTexto}"`;
        contenedorFrases.appendChild(p);
      });
    } else {
      contenedorFrases.textContent = 'Sin frase famosa';
    }
    ...
    bloqueTexto.appendChild(edad);
    bloqueTexto.appendChild(fechaNacimiento);
    bloqueTexto.appendChild(genero);
    bloqueTexto.appendChild(ocupacion);
    bloqueTexto.appendChild(contenedorFrases);
